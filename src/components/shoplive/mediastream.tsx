...
export const MediaStream = ({
        ...
        janus = new Janus({
            ...
            success:()=>{
                janus.attach({
                    ...
                    onlocalstream: (stream) => {
                        ...
                        setState((s) => ({
                            ...s,
                            videosrckey: Math.floor(Math.random() * 1000) + 1,
                        }));
                    },
                    onremotestream: (stream) => {},
            ...
            destroyed: () => {
                ...
                if (publisher && published.current) {
                    // const convertVideoUrl = `${constants.STREAM_MEDIA_URI}/convert?filename=${channelRoomID}`;
                    // console.log(
                    //     'convertVideoUrl',
                    //     `http://18.185.121.9:5000/${channelRoomID}-record.mp4`,
                    // );
                    // try {
    ...
    return (
        <div>
            <video id="localvideo" ref={videoref} key={videosrckey} autoPlay style={{width:'100%',height:'100%'}}></video>
        </div>
    )
}